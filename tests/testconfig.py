...
class Config_openmpi(MachineConfig):
    ...
    def getInstallDir(self):
        if "openmpi_install" not in configOverride:
            return os.path.join("/usr", "apps", "geosx", "packages.icc91-x86-64-openmpi-tcp", "openmpi", "1.4.3")
        else:
...
class Config_winParallel(MachineConfig):
    ...
    def setGeosxPath(self, defaultSubdir=""):
        ...
        if "executable_path" not in configOverride:
            if "--nompi" in options.ats:
                ...
                config.executable_path = os.path.realpath(
                    os.path.join(self.clientroot, "src", "Win32", "x64", "ParRelease"))
    ...
    def setup(self):
        ...
        if "windows_mpiexe" not in configOverride:
            config.windows_mpiexe = os.getenv(
                "VISIT_MPIEXEC",
...
def handleMachineType():
    ...
    SupportedMachines = (Config_GeosxAtsSlurmProcessorScheduled, Config_SlurmProcessorScheduled, Config_nersc,
                         Config_dawnHTC, Config_bgqos_0, Config_lassen, Config_summit, Config_darwin,
                         Config_winParallel, Config_cray, Config_openmpi)
